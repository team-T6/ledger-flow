...
- **단계별 "칸 폴더"가 repo 최상위에 있다**: `collect/` `refine/` `verify1/` `verify2/` `verify3/` `merge/` `orchestrator/` — 담당자별 1칸
  ...
  - [MUST] 칸별 `result.*`(및 `archive/`)는 실행마다 재생성되는 산출물이라 **커밋하지 않는다** (`.gitignore`) — 커밋되는 산출물은 Pages 데모용 `web/demo/` 고정 샘플뿐. 견본·문서가 값을 참조할 땐 커밋되는 `sample_data/`를 기준으로 쓴다
  - [MUST] 칸 폴더를 옮기지 않고, 견본·문서(`README.md`·`input-sample.md`·`stub.md`)와 생성 코드는 지우지 않는다. 새 산출물은 견본 옆에 만든다
