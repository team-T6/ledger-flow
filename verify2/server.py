"""verify2 화면(screen.html)과 call-agent.py를 이어주는 가벼운 로컬 서버.

표준 라이브러리(http.server)만 쓴다 — 추가 설치 없이 동작.
screen.html의 "검증하기" 버튼이 POST /call-agent 로 입력 글을 보내면,
call-agent.py의 call_verify2_agent()를 그대로 불러 처리 결과를 돌려준다.
API 키는 이 서버가 아니라 call-agent.py 쪽에서 .env를 읽어 쓴다.

사용법: python3 verify2/server.py  (그다음 verify2/screen.html을 브라우저로 연다)
"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not input_text:
            ...
            return
        try:
            result = call_agent.call_verify2_agent(input_text)
...
if __name__ == "__main__":
    ...
    print(f"verify2 담당자 호출 서버 실행 중 — http://localhost:{PORT}")
    print("verify2/screen.html을 브라우저로 열어 사용하세요. 끄려면 Ctrl+C.")
    try:
