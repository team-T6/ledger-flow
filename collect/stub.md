...
> 컬럼은 [interface-spec.md](../docs/interface-spec.md) "거래 표 스키마"(날짜·지출·수익·결제처·비고·결제수단·결제자, 카테고리 제외)에 수집 단계 내부에서 필요한 `transaction_id`·`source_type`·`collect_status`를 더한 모양이다 — 정본·순서는 [docs/agents/collect.md](../docs/agents/collect.md) §2 참고. 값은 `sample_data/hana_card/`의 더미 하나카드 데이터를 그대로 가져다 썼다 — 실데이터 아님.
>
...
| transaction_id | 날짜 | 지출 | 수익 | 결제처 | 비고 | 결제수단 | 결제자 | source_type | collect_status |
|---|---|---:|---:|---|---|---|---|---|---|
| `tx_260202_01` | 2026-02-02 | 67,302 | 0 | 쿠팡 | 포인트 적립 1,346원 | 하나카드 | 개인카드 | card_excel | 확인됨 |
...
| transaction_id | 날짜 | 지출 | 결제처 | 비고 | source_type | collect_status |
|---|---|---:|---|---|---|---|
...
| transaction_id | 날짜 | 지출 | 결제처 | 결제수단 | source_type | collect_status |
|---|---|---:|---|---|---|---|
