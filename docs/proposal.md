...
flowchart TD
    ...
    subgraph PIPELINE["월말결산 파이프라인"]
        ...
        MERGE["<div style='text-align:left'><b style='color:#1E5B45'>MERGE(통합)</b><br/><span style='color:#1C1917'>반려·대상외 건을 빼고 장부·리포트 작성</span></div>"]
...
필터 책임도 검증 한 곳에 모읍니다. 수집은 대상 월 위주로 모으되 걸러내지는 않고, 다른 달 거래가 섞여 들어오면 기간·금액 검증이 `대상외`로 판정해 지휘가 사람 확인 없이 결산에서 뺍니다 — 사용자가 판단할 게 없는 건은 확인 목록에 올리지 않습니다.
...
- **결산 진행 중**에는 중간 확인 화면에서 값을 바로 고치거나, 명의를 알 수 없어 미확정으로 남은 결제구분을 정하거나, 결산에서 뺄 건을 제외할 수 있습니다
...
| Google Drive 연동 | 계정 폴더를 탐색해 가져올 파일을 골라 업로드와 동일 경로로 처리 |
| 카드 구분 등록 | 결제수단명(또는 원본 파일명) → 개인/법인 매핑을 등록해 두면, 원본만으로 명의를 알 수 없는 거래의 결제구분을 자동으로 채움 |
| 진행 현황 | 결산 진행률 실시간 표시, 단계별 처리 건수. 새로고침·다른 창 접속 시 진행 화면 복원 |
| 중간 확인 | 결산 도중 "확인 필요" 건을 화면에서 수정·확정 — 값 수정, 미확정 결제구분 확정, 결산 제외 |
...
- **기간**: 2026-08-05(최초 커밋) - 2026-09-02(현재까지) — 총 222개 커밋
- **파이프라인**: 수집·가공·검증(분류/기간·금액/부정 사용)·통합·지휘 7개 단계 전부 실제 Claude 에이전트 연동 완료
- **웹**: 초대코드 인증, 업로드, Google Drive 가져오기, 카드 구분 등록, 진행 현황, 중간 확인, 결산 서고, 재결산, 분류 기준 설정까지 실제 실행 모드로 동작
- **데모**: GitHub Pages에 상시 공개 — 브라우저에서 결산 흐름을 바로 체험 가능

## 7. 시연 계획

- **GitHub Pages 데모** — 설치 없이 브라우저에서 결산 흐름을 둘러보는 쇼케이스, 중간 확인 단계까지 체험 가능 (`demo` 브랜치의 `web/` 변경 시 자동 배포)
