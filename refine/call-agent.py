"""refine 담당자(Claude)를 실제로 불러 쓰는 자리.

.claude/agents/refine.md에 적힌 역할 지시문을 시스템 프롬프트로 그대로 쓰고,
화면에서 넘어온 글 한 덩어리를 사용자 메시지로 보내 처리 결과를 받는다.
API 키는 팀 폴더 .env의 ANTHROPIC_API_KEY를 읽어 쓴다 (키 값은 이 파일에 적지 않는다).

사용법: echo "<처리할 글>" | python3 refine/call-agent.py
"""
...
AGENT_SPEC_PATH = os.path.join(REPO_ROOT, ".claude", "agents", "refine.md")
...
def call_refine_agent(input_text: str) -> str:
...
if __name__ == "__main__":
    ...
    if not input_text:
        ...
        print(call_refine_agent(input_text))
