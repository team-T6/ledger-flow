...
| `tx_260224_01` | 2026-02-24 | 배달의민족 | 식대 | 반려 | 카테고리 "식대"는 체계에 없음 — 계정과목명으로 보임, "식비"로 추정 |
| `tx_260226_01` | 2026-02-26 | 스타벅스 강남점 | 카페 | 반려 | 카테고리 "카페"는 체계에 없음 — "카페·간식"으로 추정 |
| `tx_260207_01` | 2026-02-07 | 무신사 | | 반려 | 카테고리가 비어 있어 판정 불가 (가공에서 "확인 필요" 처리된 건) |

`tx_260203_01`은 명칭이 체계에 있으므로 통과다. 영화관 지출에 `기타 지출`이 어울리는지는 이 칸이 정하지 않는다 — 어긋나 보이면 반려 대신 `verify1_reason`에 의견만 남긴다.
...
```json
{
  ...
  "flags": [
    ...
    { "row": 4, "type": "반려", "reason": "카테고리 \"카페\"는 체계에 없음 — \"카페·간식\"으로 추정" },
