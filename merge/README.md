...
가공(refine)이 만든 거래 표와 분류 검증([verify1/](../verify1/))·기간·금액 검증([verify2/](../verify2/))의 항목별 통과/반려 결과를 모아, 엑셀 회계장부와 PDF 결산 리포트를 한 묶음으로 만든다. 파이프라인의 마지막 산출 단계다.

## 받는 것 / 내놓는 것

| | 내용 | 모양 |
|---|---|---|
| 받는 것 | 가공된 거래 표 + 분류/기간·금액 검증 결과 | [input-sample.md](input-sample.md) |
...
- `sample_data/` 기준 가공된 거래 표 예시 + 분류/기간·금액 검증 결과 예시를 넣어 엑셀 장부·PDF 리포트가 정해진 컬럼·목차대로 나오는지 확인한다
- 반려·미완 케이스를 담은 별도 샘플로 "확인 필요" 항목에 제대로 실리는지 확인한다

## 협의 중 / 남은 과제

- ~~PDF 리포트 목차·엑셀 장부 컬럼이 "초안" 상태~~ — 해소(확정): 목차는 개정안 그대로, 장부 컬럼은 거래 표 스키마와 동일로 확정. 계정과목 컬럼은 도입하지 않아 `사용료` 판별 규칙도 불필요 — `카테고리`가 `결제구분`에 맞는 체계 값이면 충분 (interface-spec.md "산출물 양식"·확정 로그)
