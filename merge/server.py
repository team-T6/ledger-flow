"""merge 화면(screen.html)과 call-agent.py를 이어주는 가벼운 로컬 서버.

표준 라이브러리(http.server)만 쓴다 — 추가 설치 없이 동작.
screen.html의 "부르기" 버튼이 POST /call-agent 로 입력 글을 보내면,
call-agent.py의 call_merge_agent()를 그대로 불러 처리 결과를 돌려준다.
